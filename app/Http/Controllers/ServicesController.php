...
class ServicesController extends Controller
{
    public function index()
    {
        ...
        if($role->hasPermissionTo('service-index')){
            $permissions = Role::findByName($role->name)->permissions;
            foreach ($permissions as $permission)
                $all_permission[] = $permission->name;
            if(empty($all_permission))
                $all_permission[] = 'dummy text';
            return view('services.index', compact('all_permission'));
        }
        else
    ...
    public function productData(Request $request)
    {
        ...
        if(empty($request->input('search.value'))){
            $products = Product::with( 'brand', 'unit')->offset($start)
        ...
        else
        {
            ...
            $products =  Product::select('products.*')->where('type', 'digital')
                        ->with( 'brand', 'unit')
            ...
            $totalFiltered = Product::leftjoin('brands', 'products.brand_id', '=', 'brands.id')
                            ->where('type', 'digital')
                            ->where([
                                ['products.name','LIKE',"%{$search}%"],
                                ['products.is_active', true],
...
//                $nestedData['category'] = $product->category->name;
                ...
                if(in_array("service-edit", $request['all_permission']))
                    $nestedData['options'] .= '<li>
                            <a href="'.route('services.edit', ['id' => $product->id]).'" class="btn btn-link"><i class="fa fa-edit"></i> '.trans('file.edit').'</a>
                        </li>';
                ...
                $nestedData['product'] = array( '[ "'.$product->type.'"', ' "'.$product->name.'"', ' "'.$product->code.'"', ' "'.$nestedData['brand'].'"', ' "'.$nestedData['unit'].'"', ' "'.$product->cost.'"', ' "'.$product->price.'"', ' "'.$tax.'"', ' "'.$tax_method.'"', ' "'.$product->alert_quantity.'"', ' "'.preg_replace('/\s+/S', " ", $product->product_details).'"', ' "'.$product->id.'"', ' "'.$product->product_list.'"', ' "'.$product->qty_list.'"', ' "'.$product->price_list.'"', ' "'.$product->qty.'"', ' "'.$product->image.'"]'
                );
    ...
    public function edit($id)
    {
        ...
        if ($role->hasPermissionTo('products-edit')) {
            ...
            return view('services.edit',compact('lims_product_list', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_product_data', 'lims_product_variant_data'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function updateProduct(Request $request)
    {
        if(!env('USER_VERIFIED')) {
            ...
            \Session::flash('edit_message', 'تم تعديل بيانات الخدمة بنجاح');
        }
    }
    ...
    public function destroy($id)
    {
        ...
        $lims_product_data->save();
        return redirect('services')->with('message', 'تم حذف الخدمة بنجاح');
    }
}
